...
@bot.event
async def on_ready():
    print('I am Cersei Lannister, Queen of the Andals.')
    print("I rule in ", len(bot.guilds), "servers:")
    n = len(bot.guilds)
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Activity(type=discord.ActivityType.watching, name="over The Seven Kingdoms."))
    print('House Lannister is ready.'.format(bot))
...
async def on_application_command_error(ctx: discord.ApplicationContext, error):
    ...
    if isinstance(error, discord.HTTPException) or isinstance(error, discord.errors.NotFound):
        ...
        await ctx.send("Oh no! An unknown error occurred! Contact Klaus Noxix#9999, and he might be able to help you out.")
        await debug_channel.send(f'**Exception raised!**\nAuthor: {ctx.author}\nServer: {ctx.guild}\nCommand: {ctx.command}\n\nError:```{error}```')

@bot.slash_command(
    name="who",
    description="Get more information about someone's nation",
    guild_ids=[944401098490392626, 926310270069006436, 838532560542761000]
)
async def who(
    ...
    embed = discord.Embed(title=nation['nation_name'], url=f"https://politicsandwar.com/nation/id={nation['id']}", color=0xD30707)
...
@bot.slash_command(name="ping", description="Pong!", guild_ids=[944401098490392626, 926310270069006436])
...
@bot.slash_command(
    ...
    guild_ids=[944401098490392626, 926310270069006436]
)
async def verify(
    ctx: discord.ApplicationContext,
